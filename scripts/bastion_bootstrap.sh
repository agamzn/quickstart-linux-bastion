...
function setup_environment_variables() {
    ...
    # TODO: Syslog Path / Local5 / etc here.
    export REGION ETHO_MAC EIP_LIST INSTANCE_ID
...
function configure_bash_commands_to_syslog() {
    echo "${FUNCNAME[0]} Started"

    bash_include_file="${1}"
    cat <<-EOF >> ${bash_include_file}
        PROMPT_COMMAND=$(history -a)
        typeset -r PROMPT_COMMAND

        function log2syslog
        {
           declare SSH_CONNECTING_IP
           declare command
...
function harden_ssh_security () {
    ...
    #Allow ssh to instance and log connection
       ...
       # This could be circumvented through other commands (nc, bash, etc.)
       # We intentionally prevent users from supplying other commands to mitigate this.
...
function amazon_os () {
    ...
    # Maintaining SSH_CLIENT variable through sudo.
    echo -e "\nDefaults env_keep += \"SSH_CLIENT\"" >>/etc/sudoers

    # Housekeeping for the custom shell.
    chown root:ec2-user /usr/bin/script
    service sshd restart

    # Installing CWLogs.
    qs_cloudwatch_install
    qs_cloudwatch_tracklog /var/log/bastion/bastion.log
    ...
    echo "0 0 * * * yum -y update --security" >> /tmp/security-update-cron
    crontab /tmp/security-update-cron && rm /tmp/security-update-cron
...
function ubuntu_os () {
    ...
    qs_cloudwatch_install
    qs_cloudwatch_tracklog /var/log/bastion/bastion.log
    qs_cloudwatch_tracklog /var/log/syslog

    # Run security updates
    apt-get install unattended-upgrades
    echo "0 0 * * * unattended-upgrades -d" >> /tmp/security-update-cron
    crontab /tmp/security-update-cron ** rm /tmp/security-update-cron
